  ...
  const mockBrowserWindow = {
    ...
    webContents: {
      executeJavaScript: vi.fn().mockImplementation((script: string) => {
        ...
        if (script.includes('.builder__header__description')) return Promise.resolve("Rob's Cpt. America");
        if (script.includes('.builder__header__icon')) return Promise.resolve("builder__header__icon Paladin");

        if (script.includes('.builder__navigation__link')) return Promise.resolve();

        if (script.includes('.build__skill__wrapper')) {
          return Promise.resolve([
            { skillName: 'Blessed Shield', points: 1, maxPoints: 1, tier: 'active', nodeType: 'active' },
            { skillName: 'Consecration', points: 1, maxPoints: 1, tier: 'active', nodeType: 'active' }
          ]);
        }

        if (script.includes('.skill__tree__item--active')) {
          return Promise.resolve([
            { skillName: 'Blessed Shield', points: 5, maxPoints: 5, tier: 'core', nodeType: 'active' },
            { skillName: 'Enhanced Blessed Shield', points: 1, maxPoints: 1, tier: 'core', nodeType: 'passive' }
          ]);
        }
        ...
        if (script.includes('.paragon__board')) {
          ...
          return Promise.resolve([
            {
              boardName: 'Warbringer', boardIndex: 0, glyph: { glyphName: 'Spirit', level: 15 },
              allocatedNodes: [{ nodeName: 'Str', nodeType: 'normal', allocated: true, row: 5, col: 5 }],
              boardRotation: 90, boardBgUrl: '', boardX: 0, boardY: 0, tileCount: 3
            },
            {
              boardName: 'Blood Rage', boardIndex: 1, glyph: null,
              allocatedNodes: [], boardRotation: 0, boardBgUrl: '', boardX: 0, boardY: 0, tileCount: 0
            }
          ]);
        }
        ...
        if (script.includes('.builder__gear__item')) {
          ...
          return Promise.resolve([
            ...
            { slot: 'Chest Armor', itemName: 'Mantle of the Grey', itemType: 'Unique', socketedGems: [] }
          ]);
        }
        ...
        if (script.includes('.builder__stats__group')) {
          return Promise.resolve([
            {
              slot: 'Helm',
              affixes: [{ name: '45% Critical Strike Chance', isGreater: false }, { name: '+15% Cooldown Reduction', isGreater: false }],
              implicitAffixes: [], temperedAffixes: [],
              greaterAffixes: [{ name: '+120 Strength', isGreater: true }],
              rampageEffect: null, feastEffect: null
            },
            { slot: 'Chest Armor', affixes: [], implicitAffixes: [], temperedAffixes: [], greaterAffixes: [], rampageEffect: null, feastEffect: null }
          ]);
        }

        if (script.includes('.codex__tooltip')) { // Phase C gear aspect
          return Promise.resolve([]); 
        }

        if (script.includes('.builder__gems')) { // Runes
...
const electronMock = vi.hoisted(() => ({
  BrowserWindow: class { constructor() { return mockBrowserWindow; } },
...
describe('D4BuildsScraper', () => {
  ...
  it('should handle d4builds.gg URLs', () => {
    ...
    expect(scraper.canHandle('https://d4builds.gg/builds/blessed-shield-paladin-endgame/?var=0')).toBe(true)
