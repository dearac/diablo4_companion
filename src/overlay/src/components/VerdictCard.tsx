    ...
    if (result.error) {
        return (
            ...
            >
                ...
                <button className="verdict-card__dismiss" onClick={handleDismiss}>✕</button>
            </div>
        )
    }
        ...
        >
            ...
            {verdict.socketDelta !== 0 && (
                <div className="verdict-card__sockets">
                    💎 Sockets: {verdict.scannedItem.sockets}/{verdict.scannedItem.sockets - verdict.socketDelta}
                    {verdict.socketDelta < 0 && (
            ...
            {verdict.greaterAffixCount > 0 && (
                <div className="verdict-card__greater">
                    ⭐ Greater Affixes: {verdict.greaterAffixCount}
                </div>
            )}
                    ...
                    {verdict.recommendations.map((rec, i) => (
                        <div key={i} className="verdict-card__rec">
                            ...
                            <span className="verdict-card__rec-text">
                                {rec.action === 'enchant' && (
                                    <>
                                        <strong>ENCHANT:</strong> Reroll &quot;{rec.removeAffix}&quot; at {rec.vendor} → try for &quot;{rec.addAffix}&quot;
                                    </>
                                )}
                                {rec.action === 'temper' && (
                                    <>
            ...
            {verdict.equippedComparison && (
                <div className={`verdict-card__equipped ${verdict.equippedComparison.isUpgrade ? 'verdict-card__equipped--upgrade' : 'verdict-card__equipped--downgrade'}`}>
                    {verdict.equippedComparison.isUpgrade ? '⬆️' : '⬇️'} vs. Equipped:
                    {verdict.equippedComparison.isUpgrade ? ' UPGRADE' : ' DOWNGRADE'}
                    {' '}({verdict.equippedComparison.equippedMatchCount}/{verdict.buildTotalExpected} → {verdict.buildMatchCount}/{verdict.buildTotalExpected})
                </div>
            )}
        </div>
    )
